...
from gevent.timeout import Timeout # Import Timeout for gevent
...
# We use the non-streaming endpoint with aiohttp for simplicity, streaming is handled by our app
...
def stream(ws):
    ...
    async def main_loop():
        """The main async loop to be run for each connection."""
        while ws.connected:
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(full_url, headers=headers, json=payload) as resp:
                        ...
                        # Process the streaming response from aiohttp
                        async for line in resp.content:
                            ...
                            if decoded_line.startswith('data: '):
                                try:
                                    ...
                                    # This can happen with incomplete stream chunks, it's safe to ignore
                                    pass
    ...
    try:
        ...
        print("Main loop finished. Closing connection.")
